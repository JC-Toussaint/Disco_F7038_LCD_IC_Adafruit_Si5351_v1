...
static bool Si5351_write_u8(Si5351_ConfigTypeDef *handle,  uint8_t reg_address, uint8_t reg_data)
{
	uint8_t data[] = {reg_data};
    return HAL_I2C_Mem_Write( handle->i2c_handle, handle->device_address, reg_address,
                           0x1, data, 0x1, SI5351_TIMEOUT) == HAL_OK;
}

static bool Si5351_write_data(Si5351_ConfigTypeDef *handle, uint8_t reg_address, uint8_t *data, size_t length)
{
    if (length == 0 || length > 4) {
        return false;
    }
    return HAL_I2C_Mem_Write( handle->i2c_handle, handle->device_address, reg_address,
                           0x1, data, length, SI5351_TIMEOUT) == HAL_OK;
}

static bool Si5351_read_u8(Si5351_ConfigTypeDef *handle, uint8_t reg_address, uint8_t *dest)
{
    return HAL_I2C_Mem_Read( handle->i2c_handle, handle->device_address, reg_address,
                           0x1, dest, 0x1, SI5351_TIMEOUT) == HAL_OK;
}
...
void Si5351_StructInit(Si5351_ConfigTypeDef *Si5351_ConfigStruct)
{
	...
	Si5351_ConfigStruct->device_address = SI5351_I2C_ADDRESS;
	Si5351_ConfigStruct->i2c_handle     = SI5351_I2C_HANDLE;
	...
	for (i=0; i<=7; i++)
	{
		...
		Si5351_ConfigStruct->CLK[i].CLK_I_Drv = CLK_I_Drv_8mA; // CLK_I_Drv_8mA;
...
void Si5351_OSCConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct)
{
	...
	Si5351_read_u8(Si5351_ConfigStruct, REG_XTAL_CL, &tmp);
	...
	Si5351_write_u8(Si5351_ConfigStruct, REG_XTAL_CL, tmp);

	//set CLKIN pre-divider
	Si5351_read_u8(Si5351_ConfigStruct, REG_CLKIN_DIV, &tmp);

	tmp &= ~CLKIN_MASK;
	tmp |= CLKIN_MASK & Si5351_ConfigStruct->OSC.CLKIN_Div;
	Si5351_write_u8(Si5351_ConfigStruct, REG_CLKIN_DIV, tmp);

	//set fanout of XO, MS0, MS4 and CLKIN - should be always on unless you
	//need to reduce power consumption
    Si5351_read_u8(Si5351_ConfigStruct, REG_FANOUT_EN, &tmp);
	...
	Si5351_write_u8(Si5351_ConfigStruct, REG_FANOUT_EN, tmp);
	...
	Si5351_write_u8(Si5351_ConfigStruct, REG_VCXO_PARAM_0_7, tmp);
	tmp = (uint8_t)(VCXO_Param>>8);
	Si5351_write_u8(Si5351_ConfigStruct, REG_VCXO_PARAM_8_15, tmp);
	tmp = (uint8_t)((VCXO_Param>>16) & VCXO_PARAM_16_21_MASK);
	Si5351_write_u8(Si5351_ConfigStruct, REG_VCXO_PARAM_16_21, tmp);
}

EnableState Si5351_CheckStatusBit(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_StatusBitTypeDef StatusBit)
{
	uint8_t tmp;
	Si5351_read_u8(Si5351_ConfigStruct, REG_DEV_STATUS, &tmp);
	tmp &= StatusBit;
	return tmp;
}
...
{
	uint8_t tmp;
	Si5351_read_u8(Si5351_ConfigStruct, REG_DEV_STICKY, &tmp);
	tmp &= StatusBit;
	return tmp;
}
...
{
	uint8_t tmp;
	Si5351_read_u8(Si5351_ConfigStruct, REG_INT_MASK, &tmp);
	...
	Si5351_write_u8(Si5351_ConfigStruct, REG_INT_MASK, tmp);
}
...
void Si5351_ClearStickyBit(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_StatusBitTypeDef StatusBit)
{
	...
	Si5351_read_u8(Si5351_ConfigStruct, REG_DEV_STICKY, &tmp);
	tmp &= ~StatusBit;
	Si5351_write_u8(Si5351_ConfigStruct, REG_DEV_STICKY, tmp);
}
...
void Si5351_PLLConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_PLLChannelTypeDef PLL_Channel)
{
	...
	Si5351_read_u8(Si5351_ConfigStruct, REG_PLL_CLOCK_SOURCE, &tmp);
	tmp_mask = PLLA_CLOCK_SOURCE_MASK << PLL_Channel;
	tmp &= ~tmp_mask;
	tmp |= tmp_mask & Si5351_ConfigStruct->PLL[PLL_Channel].PLL_Clock_Source;
	Si5351_write_u8(Si5351_ConfigStruct, REG_PLL_CLOCK_SOURCE, tmp);
	...
	if ((Si5351_ConfigStruct->PLL[PLL_Channel].PLL_Multiplier_Numerator != 0) | 
	   ((Si5351_ConfigStruct->PLL[PLL_Channel].PLL_Multiplier_Integer & 0x01) != 0 )){
		Si5351_read_u8(Si5351_ConfigStruct, REG_FB_INT + PLL_Channel, &tmp);
		tmp &= ~FB_INT_MASK;
		Si5351_write_u8(Si5351_ConfigStruct, REG_FB_INT + PLL_Channel, tmp);
	}
	...
	tmp = (uint8_t) MSN_P1;
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P1_0_7 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P1 >> 8);
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P1_8_15 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P1_16_17_MASK & (MSN_P1 >> 16));
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P1_16_17 + 8 * PLL_Channel, tmp);

	tmp = (uint8_t) MSN_P2;
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P2_0_7 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P2 >> 8);
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P2_8_15 + 8 * PLL_Channel, tmp);

	Si5351_read_u8(Si5351_ConfigStruct, REG_MSN_P2_16_19, &tmp);
	tmp &= ~MSN_P2_16_19_MASK;
	tmp |= (uint8_t) (MSN_P2_16_19_MASK & (MSN_P2 >> 16));
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P2_16_19 + 8 * PLL_Channel, tmp);

	tmp = (uint8_t) MSN_P3;
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P3_0_7 + 8 * PLL_Channel, tmp);
	tmp = (uint8_t) (MSN_P3 >> 8);
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P3_8_15 + 8 * PLL_Channel, tmp);

	Si5351_read_u8(Si5351_ConfigStruct, REG_MSN_P3_16_19, &tmp);
	tmp &= ~MSN_P3_16_19_MASK;
	tmp |= (uint8_t) (MSN_P3_16_19_MASK & ((MSN_P3 >> 16) << 4));
	Si5351_write_u8(Si5351_ConfigStruct, REG_MSN_P3_16_19 + 8 * PLL_Channel, tmp);
...
void Si5351_PLLReset(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_PLLChannelTypeDef PLL_Channel)
{
	...
	//reset PLL
	Si5351_read_u8(Si5351_ConfigStruct, REG_PLL_RESET, &tmp);
	if (PLL_Channel == PLL_A){
		tmp |= PLLA_RESET_MASK;
	} else {
		tmp |= PLLB_RESET_MASK;
	}
	Si5351_write_u8(Si5351_ConfigStruct, REG_PLL_RESET, tmp);
}
...
void Si5351_SSConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct)
{
	...
	{
		Si5351_read_u8(Si5351_ConfigStruct, REG_SSC_EN, &tmp);
		tmp &= ~SSC_EN_MASK;
		Si5351_write_u8(Si5351_ConfigStruct, REG_SSC_EN, tmp);
	}

	//set default value of SS_NCLK - spread spectrum reserved register
	Si5351_read_u8(Si5351_ConfigStruct, REG_SS_NCLK, &tmp);
	tmp &= ~SS_NCLK_MASK;
	tmp |= SS_NCLK_MASK & (Si5351_ConfigStruct->SS.SS_NCLK);
	Si5351_write_u8(Si5351_ConfigStruct, REG_SS_NCLK, tmp);

	//set SS mode
	Si5351_read_u8(Si5351_ConfigStruct, REG_SSC_MODE, &tmp);
	tmp &= ~SSC_MODE_MASK;
	tmp |= SSC_MODE_MASK & Si5351_ConfigStruct->SS.SS_Mode;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSC_MODE, tmp);
	...
	//set SSUDP parameter
	tmp = (uint8_t) SSUDP;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUDP_0_7, tmp);
	Si5351_read_u8(Si5351_ConfigStruct, REG_SSUDP_8_11, &tmp);
	tmp &= ~SSUDP_8_11_MASK;
	tmp |= (uint8_t) (SSUDP_8_11_MASK & ((SSUDP >> 8) << 4));
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUDP_8_11, tmp);
	...
	tmp = (uint8_t) SSUP_P1;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUP_P1_0_7, tmp);

	Si5351_read_u8(Si5351_ConfigStruct, REG_SSUP_P1_8_11, &tmp);
	tmp &= ~SSUP_P1_8_11_MASK;
	tmp |= (uint8_t)(SSUP_P1_8_11_MASK & (SSUP_P1 >> 8));
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUP_P1_8_11, tmp);

	//write SSUP parameter P2
	tmp = (uint8_t) SSUP_P2;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUP_P2_0_7, tmp);

	Si5351_read_u8(Si5351_ConfigStruct, REG_SSUP_P2_8_14, &tmp);
	tmp &= ~SSUP_P2_8_14_MASK;
	tmp |= (uint8_t)(SSUP_P2_8_14_MASK & (SSUP_P2 >> 8));
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUP_P2_8_14, tmp);

	//write SSUP parameter P3
	tmp = (uint8_t) SSUP_P3;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUP_P3_0_7, tmp);

	Si5351_read_u8(Si5351_ConfigStruct, REG_SSUP_P3_8_14, &tmp);
	tmp &= ~SSUP_P3_8_14_MASK;
	tmp |= (uint8_t)(SSUP_P3_8_14_MASK & (SSUP_P3 >> 8));
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSUP_P3_8_14, tmp);

	//write SSDN parameter P1
	tmp = (uint8_t) SSDN_P1;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSDN_P1_0_7, tmp);
	Si5351_read_u8(Si5351_ConfigStruct, REG_SSDN_P1_8_11, &tmp);
	tmp &= ~SSDN_P1_8_11_MASK;
	tmp |= (uint8_t)(SSDN_P1_8_11_MASK & (SSDN_P1 >> 8));
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSDN_P1_8_11, tmp);

	//write SSDN parameter P2
	tmp = (uint8_t) SSDN_P2;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSDN_P2_0_7, tmp);
	Si5351_read_u8(Si5351_ConfigStruct, REG_SSDN_P2_8_14, &tmp);
	tmp &= ~SSDN_P2_8_14_MASK;
	tmp |= (uint8_t)(SSDN_P2_8_14_MASK & (SSDN_P2 >> 8));
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSDN_P2_8_14, tmp);

	//write SSDN parameter P3
	tmp = (uint8_t) SSDN_P3;
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSDN_P3_0_7, tmp);
	Si5351_read_u8(Si5351_ConfigStruct, REG_SSDN_P3_8_14, &tmp);
	tmp &= ~SSDN_P3_8_14_MASK;
	tmp |= (uint8_t)(SSDN_P3_8_14_MASK & (SSDN_P3 >> 8));
	Si5351_write_u8(Si5351_ConfigStruct, REG_SSDN_P3_8_14, tmp);
...
void Si5351_MSConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_MSChannelTypeDef MS_Channel)
{
	...
	Si5351_read_u8(Si5351_ConfigStruct, REG_MS_SRC + MS_Channel, &tmp);
	tmp &= ~MS_SRC_MASK;
	if (Si5351_ConfigStruct->MS[MS_Channel].MS_Clock_Source == MS_Clock_Source_PLLB)
	{
		tmp |= MS_SRC_MASK;
	}
	Si5351_write_u8(Si5351_ConfigStruct, REG_MS_SRC + MS_Channel, tmp);
	...
	if (MS_Channel <= MS5) //configuration is simpler for MS6 and 7 since they are integer-only
	{
		...
		{
			Si5351_read_u8(Si5351_ConfigStruct, REG_MS_DIVBY4 + 8 * MS_Channel, &tmp);
			tmp &= ~MS_DIVBY4_MASK;
			Si5351_write_u8(Si5351_ConfigStruct, REG_MS_DIVBY4 + 8 * MS_Channel, tmp);
		}
		...
		{
			Si5351_read_u8(Si5351_ConfigStruct, REG_MS_INT + MS_Channel, &tmp);
			tmp &= ~MS_INT_MASK;
			Si5351_write_u8(Si5351_ConfigStruct, REG_MS_INT + MS_Channel, tmp);
		}
		...
		tmp = (uint8_t) MS_P1;
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P1_0_7 + 8 * MS_Channel, tmp);
		tmp = (uint8_t) (MS_P1 >> 8);
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P1_8_15 + 8 * MS_Channel, tmp);
		Si5351_read_u8(Si5351_ConfigStruct, REG_MS_P1_16_17, &tmp);
		tmp &= ~MS_P1_16_17_MASK;
		tmp |= (uint8_t) (MS_P1_16_17_MASK & (MS_P1 >> 16));
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P1_16_17 + 8 * MS_Channel, tmp);

		tmp = (uint8_t) MS_P2;
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P2_0_7 + 8 * MS_Channel, tmp);
		tmp = (uint8_t) (MS_P2 >> 8);
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P2_8_15 + 8 * MS_Channel, tmp);
		Si5351_read_u8(Si5351_ConfigStruct, REG_MS_P2_16_19 + 8 * MS_Channel, &tmp);
		tmp &= ~MS_P2_16_19_MASK;
		tmp |= (uint8_t) (MS_P2_16_19_MASK & (MS_P2 >> 16));
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P2_16_19 + 8 * MS_Channel, tmp);

		tmp = (uint8_t) MS_P3;
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P3_0_7 + 8 * MS_Channel, tmp);
		tmp = (uint8_t) (MS_P3 >> 8);
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P3_8_15 + 8 * MS_Channel, tmp);
		Si5351_read_u8(Si5351_ConfigStruct, REG_MS_P3_16_19 + 8 * MS_Channel, &tmp);
		tmp &= ~MS_P3_16_19_MASK;
		tmp |= (uint8_t) (MS_P3_16_19_MASK & ((MS_P3 >> 16) << 4));
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS_P3_16_19 + 8 * MS_Channel, tmp);
		...
		Si5351_write_u8(Si5351_ConfigStruct, REG_MS67_P1 + (MS_Channel - MS6), (uint8_t)(Si5351_ConfigStruct->MS[MS_Channel].MS_Divider_Integer));
...
void Si5351_CLKPowerCmd(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_CLKChannelTypeDef CLK_Channel)
{
	...
	Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_DIS_STATE + (CLK_Channel >> 2), &tmp); //increment the address by 1 if CLKx>=CLK4
	...
	Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_DIS_STATE + (CLK_Channel >> 2), tmp);

	//set OEB pin
	Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_OEB, &tmp);
	...
	if (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_Enable == OFF) //disable clock
	{
		...
		Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_EN, &tmp);
		tmp |= 1 << CLK_Channel;
		Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_EN, tmp);
	}

	if (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_PowerDown == ON) //power down clock
	{
		//power down output driver
		Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_PDN + CLK_Channel, &tmp);
		tmp |= CLK_PDN_MASK;
		Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_PDN + CLK_Channel, tmp);
	}
...
void Si5351_CLKConfig(Si5351_ConfigTypeDef *Si5351_ConfigStruct, Si5351_CLKChannelTypeDef CLK_Channel)
{
	...
	Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_SRC + CLK_Channel, &tmp);
	tmp &= ~CLK_SRC_MASK;
	tmp |= CLK_SRC_MASK & Si5351_ConfigStruct->CLK[CLK_Channel].CLK_Clock_Source;
	Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_SRC + CLK_Channel, tmp);

	//set CLK inversion
	Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_INV + CLK_Channel, &tmp);
	tmp &= ~CLK_INV_MASK;
	if (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_Invert == ON)
	{
		tmp |= CLK_INV_MASK;
	}
	Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_INV + CLK_Channel, tmp);

	//set CLK current drive
	Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_IDRV + CLK_Channel, &tmp);
	tmp &= ~CLK_IDRV_MASK;
	tmp |= CLK_IDRV_MASK & Si5351_ConfigStruct->CLK[CLK_Channel].CLK_I_Drv;
	Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_IDRV + CLK_Channel, tmp);
	...
	if (CLK_Channel <= CLK5) //CLK6 and 7 are integer only, which causes several limitations
	{
		...
		Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_PHOFF + CLK_Channel, tmp);
		//set Rx divider
		Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_R_DIV + CLK_Channel * CLK_R_DIV_STEP, &tmp);
		tmp &= ~CLK_R_DIV_MASK;
		tmp |= CLK_R_DIV_MASK & (Si5351_ConfigStruct->CLK[CLK_Channel].CLK_R_Div);
		Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_R_DIV + CLK_Channel * CLK_R_DIV_STEP, tmp);
	} else {
		...
		Si5351_read_u8(Si5351_ConfigStruct, REG_CLK_R67_DIV, &tmp);
		tmp &= ~tmp_mask;
		tmp |= tmp_mask & ((Si5351_ConfigStruct->CLK[CLK_Channel].CLK_R_Div >> 4) << ((CLK_Channel-CLK6) << 2));
		Si5351_write_u8(Si5351_ConfigStruct, REG_CLK_R67_DIV, tmp);
	}
}
